    ...
    windowsPowerShellTest(
        ...
        async () => {
            ...
            const command = [
                ...
                "$script:RecordedRestUris = @()",
                "$script:RecordedWebRequestUri = ''",
                "function Invoke-RestMethod {",
                "    param([string]$Uri)",
                "    $script:RecordedRestUris += $Uri",
                "    return @{ version = '1.2.3' }",
                "}",
                "function Invoke-WebRequest {",
                "    param([string]$Uri, [string]$OutFile)",
                "    $script:RecordedWebRequestUri = $Uri",
                "    Set-Content -LiteralPath $OutFile -Value 'stub'",
                "}",
                `. '${escapePowerShellString(installScriptPath)}'`,
                "Main",
                "Write-Output ($script:RecordedRestUris -join '|')",
                "Write-Output $script:RecordedWebRequestUri",
