...
### 1. Check OpenClaw execution permissions

Run the dedicated preflight once before creating a skill:

```bash
oo skills preflight --agent openclaw
```

Treat this as the OpenClaw permission and storage probe for OpenClaw's native
...
```bash
oo skills init <name> --agent openclaw --description "..."
oo packages info "<packageName>" --json
oo search "<query>" --json
```

If OpenClaw cannot request the needed permission, or the user denies it, stop
...
Never work around a blocked `oo skills init --agent openclaw` by manually
...
Run `oo skills init <name> --agent openclaw` with a required `--description`.
...
them. If the selected OpenClaw skill directory already exists, ask for a
...
directory must come from a successful `oo skills init --agent openclaw`
