...
### 1. Check Claude execution permissions

Run the dedicated preflight once before creating a skill:

```bash
oo skills preflight --agent claude
```

Treat this as the Claude permission and storage probe for Claude's native skills
...
```bash
oo skills init <name> --agent claude --description "..."
oo packages info "<packageName>" --json
oo search "<query>" --json
```

If Claude cannot request the needed permission, or the user denies it, stop and
...
Never work around a blocked `oo skills init --agent claude` by manually creating
...
Run `oo skills init <name> --agent claude` with a required `--description`.
...
them. If the selected Claude skill directory already exists, ask for a different
...
directory must come from a successful `oo skills init --agent claude` invocation
before you fill in its workflow instructions or run validation.
