...
Treat this command as the Codex permission and storage probe. It checks that the
Codex home exists and that the configured local skill canonical storage can be
created and written. If it passes, proceed without extra permission discussion.
Do not use a different writable location when it fails.
