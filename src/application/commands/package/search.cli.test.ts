...
describe("packageSearchCommand CLI", () => {
    ...
    test("supports package search command with text output", async () => {
        ...
        try {
            ...
            expect(requests[0]?.url).toBe(
                "https://search.oomol.com/v1/packages/-/intent-search?q=image+processing&lang=en&excludeScopes=connector&excludePackages=llm",
            );
    ...
    test("adds the localized request language to package search queries", async () => {
        ...
        try {
            ...
            expect(
                new URL(requests[0]!.url).searchParams.get("excludePackages"),
            ).toBe("llm");
        }
        finally {
            await sandbox.cleanup();
